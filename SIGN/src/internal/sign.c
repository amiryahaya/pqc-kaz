...
static const char *g_level_256_phiN = "";
                  ...
                  unsigned char *hash)
{
    ...
    size_t hash_bytes = KAZ_SIGN_BYTES;  /* level-dependent: 32, 48, or 64 */
    ...
    /* SHA-256 produces 32 bytes; truncate or zero-pad to target hash_bytes */
    memset(hash, 0, hash_bytes);
    memcpy(hash, sha256_buf, 32 < hash_bytes ? 32 : hash_bytes);
                     ...
                     unsigned char *hash)
{
    ...
    if (EVP_DigestFinal_ex(rp->hash_ctx, sha256_buf, &hash_len) != 1) {
        return KAZ_SIGN_ERROR_INVALID;
    }

    /* SHA-256 produces 32 bytes; truncate or zero-pad to target hash_bytes */
    size_t target = params->hash_bytes;
    memset(hash, 0, target);
    memcpy(hash, sha256_buf, 32 < target ? 32 : target);
